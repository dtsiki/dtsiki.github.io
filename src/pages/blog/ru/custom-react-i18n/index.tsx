    ...
    <PostWrapper postConfig={POSTS_CONFIG_[EBlogPostRecord.CUSTOM_REACT_I18N]} heroConfig={HERO_CONFIG}>
      ...
      <section ref={appDesignRef}>
        ...
        <p>
          ...
          хранить язык и функцию для его переключения, а затем сможем получать к ним доступ из любого компонента в
      ...
      <section ref={contextRef}>
        ...
        <p>
          ...
          контексте. В нём будет всего лишь два свойства - выбранный язык и функция переключения языка:
        </p>
        <CodeSnippet
        ...
        <p>
          ...
          количество языков, то просто измените функцию, добавив параметр - язык, на который нужно перевести всё, а
          кнопку, например, на выпадающий список.
        </p>
        <p>
      ...
      <section>
        <h2>Переводим</h2>
        <p>Осталось дело за малым - применить всё это дело в деле.</p>
        <p>
      ...
      <section ref={conclusionRef}>
        ...
        <p className='spacer bottom large'>
          ...
          react-i18next и другие библиотеки всегда рядом и теперь вы знаете, что у них под капотом. А это, пожалуй, и
          есть главная суперсила разработчика - не просто пользоваться инструментами, а понимать, как они устроены.
        </p>
      </section>
    </PostWrapper>
  );
