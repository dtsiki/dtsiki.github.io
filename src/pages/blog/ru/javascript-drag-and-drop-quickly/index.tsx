  ...
  return (
    <PostWrapper
      ...
      <section>
        ...
        <p>
          Реализовать драг-н-дроп на JavaScript можно с помощью нативного API. Этот API так и называется — Drag and Drop
          API. Нам совершенно не нужны будут никакие библиотеки, например такие популярные как React DnD или React
          Beautiful DnD.
        </p>
        <p>
        ...
        <p className='explanation'>
          Попробуйте перетащить любую задачу внизу окна из списка задач в любую из трёх колонок «Сделать», «В процессе»
          и «Готово». Потом попробуйте перетащить задачу обратно вниз. Так и работает драг-н-дроп, который будем делать
          в этом уроке
        </p>
      ...
      <section>
        ...
        <CodeSnippet lang={ECodeLang.HTML} name='index' code={exampleDraggableElementMarkup} />
        ...
        <Window pattern={EWindowPatternVariant.SPRINKLES} type={EWindowType.CUSTOM}>
        ...
        <PostImage fileTitle='DND_UI.jpg' src={dnd_ui} alt='' />
        <p className='explanation'>Вот так это выглядит в моём браузере на моём ноутбуке</p>
        <p className='spacer top medium'>
          С перетаскиваемыми элементами разобрались, чтобы сделать элемент сбрасываемым никаких флагов вообще не надо
          добавлять, все элементы по умолчанию позволяют сбрасывать на себя перетаскиваемые элементы:
        </p>
        <CodeSnippet lang={ECodeLang.HTML} name='index' code={exampleDroppableElementMarkup} />
        ...
        <p className='spacer top medium'>
          Все элементы можно сделать перетаскиваемыми и сбрасываемыми - можно перетаскивать что угодно и сбрасывать куда
          угодно. Можно добавить ограничения и тогда не всё можно будет перетаскивать и сбрасывать куда угодно, в
      ...
      <section>
        ...
        <p className='spacer top large'>
          ...
          всего для перетаскиваемых элементов нам потребуется два события - <code className='code'>onDragStart</code> и{' '}
          <code className='code'>onDragEnd</code>.
        </p>
        <p>
          С помощью <code className='code'>onDragStart</code> будем иницилизировать процесс перетаскивания:
        </p>
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleHandlerDragStartJavaScript1} />
        ...
        <p className='spacer top large'>
          ...
          <code className='code'>onDragOver</code>. В <code className='code'>onDrop</code> будет обрабатываться сброс и
          как мы будем это делать рассмотрим чуть ниже, а вот <code className='code'>onDragOver</code> нужен будет
        ...
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleHandlerDragOverJavaScript} />
      ...
      <section>
        ...
        <p>
          ...
          будет три колонки со статусом задач - «Сделать», «В процессе» и «Готово». Эти три колонки будут зонами сброса
          или droppable элементами:
        </p>
        <DoubleCodeSnippet code={[exampleMarkUp1Html, example1CSS]} lang={[ECodeLang.HTML, ECodeLang.CSS]} />
        ...
        <DoubleCodeSnippet code={[exampleMarkUp2Html, example2CSS]} lang={[ECodeLang.HTML, ECodeLang.CSS]} />
        ...
        <p className='spacer bottom medium'>
          ...
          перетаскиваемые и сбрасываемые элементы находились на одном уровне. На самом деле эти области могут находитmся
          на совершенно разных уровнях вложенности в DOM-дереве.
        </p>
      ...
      <section>
        ...
        <p>
          ...
          <code className='code'>dataTransfer</code> - <code className='code'>setData()</code>. Он позволяет передавать
          данные между перетаскиваемым элементом и зоной сброса. <code className='code'>dataTransfer</code> может
          хранить только строки. С помощью этого метода мы запишем информацию о задаче, которую мы начали перетаскивать,
          в нашем простом примере это просто сама задача. Не будем здесь хитрить и просто передадим текст элемента.
          Достать его можно из
        ...
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleHandlerDragStartJavaScript2} />

        <p className='spacer top medium'>
          Помимо передачи данных, тут же изменим прозрачность перетаскиваемой задачи, чтобы визуально было видно какая
          из множества задач сейчас перетаскивается. Также изменение прозрачности нам поможет найти нужную задачу, когда
          нам это потребуется ниже, избавив от необходимости прописывать какие-либо дополнительные атрибуты или
        ...
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleHandlerDragEndJavaScript} />
        ...
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleHandlersDropDragOverJavaScript} />

        <p className='spacer top medium'>
          Пока что драг-н-дроп не будет работать. Мы просто записываем задачу когда начинаем перетаскивать её и читаем,
          когда сбрасываем.{' '}
        </p>
        <p>
        ...
        <ol className='list ordered'>
          <li className='list__item'>
            Когда перетащили задачу и сбросили её - убрать её оттуда, откуда её начали перетаскивать (т.е. убрать как из
            колонки так и из области задач)
          </li>
          <li className='list__item'>
        ...
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleDraggedElementJavaScript} />

        <p className='spacer top medium'>Обновим наш обработчик сброса, держа в голове ТЗ выше:</p>
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleHandlerDropJavaScript} />
        ...
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleEventListeners1JavaScript} />
        ...
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleEventListeners2JavaScript} />

        <p className='spacer top medium'>
          Собираем всё вместе. Разметка будет выглядеть так (тут ничего не изменилось с самого начала):
        </p>
        <CodeSnippet lang={ECodeLang.HTML} name='index' code={exampleMarkUpHtml} />
        ...
        <CodeSnippet lang={ECodeLang.JAVASCRIPT} name='index' code={exampleJavaScript} />
        ...
        <ReadMoreList
          items={sources}
          language={POSTS_CONFIG_[EBlogPostRecord.JAVASCRIPT_DRAG_AND_DROP_QUICKLY].language}
        />
      </section>
    </PostWrapper>
  );
