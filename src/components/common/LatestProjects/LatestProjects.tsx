...
import { EWindowPatternVariant, EWindowType } from '../Window/Window.types';
...
const LatestProjects = ({ showOnlyProjects = false }: Props): ReactElement => {
  ...
  const projects = [
    {
      ...
      title: 'Todo',
      description:
        'A simple task management application built with JavaScript library React. Redux-like manager Storeon and its localStorage module are used for state management.',
      image: todo,
    },
    {
      id: nanoid(),
    ...
    },
    {
      ...
      title: 'Tic Tac Toe',
      description:
        'My React implementation of one of the most popular board games in the world for one or two players.',
      image: ticTacToe,
    },
    {
      id: nanoid(),
      ...
      title: 'Dictionary',
      description:
        'This simple dictionary application is using real-time data from the Free Dictionary API and images from Unsplash API.',
      image: dictionary,
    },
    {
      id: nanoid(),
      ...
      title: 'Task manager',
      description:
        'A simplified kanban board inspired by Trello built with React and TypeScript. HTML drag and drop API is used here.',
      image: taskManager,
    },
  ];
  ...
  const renderWindows = useMemo(() => {
    return projects?.map((project) => {
      ...
      return (
        <div key={id} className={bind([styles.latestProjects__item, styles[variant]])}>
          <Window type={EWindowType.IMAGE} fileTitle={`${fileName}.jpg`}>
  ...
  const renderPreviews = useMemo(() => {
    return projects.map((project) => {
      ...
      return <Project key={title} title={title} description={description} variant={variant} col='col--100' />;
    });
  }, [projects]);

  return (
    <Hero variant={EHeroVariant.LIGHT}>
      <div className={styles.latestProjects}>
        <div className='container'>
          {!showOnlyProjects && <h2 className={styles.latestProjects__title}>Latest projects</h2>}
        </div>
        {isMobile ? (
          <ul className='row row--no-margin'>{renderPreviews}</ul>
        ) : (
          <div className='container'>
            <Window title='Projects' pattern={EWindowPatternVariant.SMILEY}>
              <div className={styles.latestProjects__wrapper}>{renderWindows}</div>
            </Window>
          </div>
        )}
        {!showOnlyProjects && (
          <div className={styles.latestProjects__footer}>
            <Link href='/projects'>
              <a className='link-with-tag'>
                <Tag variant={TagVariant.PRIMARY} isOutlined>
                  Check out
